...
must not create, and the gate it must pass.
"""
...
CRATE = PREAMBLE + """
## Your slice: the crate, the shared model, and `plotplot version`

Create, at the repository root: Cargo.toml (package `plotplot` 0.1.0, edition 2024, rust-version 1.85, lib + bin `plotplot`, dependencies exactly as brief §2), Cargo.lock (committed), src/main.rs, src/lib.rs, src/cli.rs, src/error.rs, src/harness.rs, src/bed.rs, src/manifest.rs, src/lock.rs, src/layout.rs, tests/version.rs, tests/fixtures/payloads/<harness>/<Event>.json, and .github/workflows/stem.yml. Add to .gitignore: target/, .loop-scratch/, .plotplot/bin/, .plotplot/beds/, .plotplot/friction/, .plotplot/receipts/ (keep every existing line).

Shapes: brief §4 for error.rs, harness.rs, bed.rs, manifest.rs, lock.rs, layout.rs, cli.rs/main.rs. The manifest schema is contracts/manifest.schema.json and the lock schema contracts/lock.schema.json; embed each with include_str! and enforce it with the `jsonschema` crate before serde parsing. `plotplot version` prints `plotplot 0.1.0` on the first line; when garden.lock exists at the root it prints `season <season>` and one line per judge, `<name> <version>` sorted by name, marking an npm judge with `(npm <package>)`. `plotplot --version` prints the same first line.

Fixture payloads, one file per harness and event, with the real field names of brief §5 and realistic values (a session id, a cwd of /work/repo, tool names the harness really uses: Claude Bash/Read/Edit/Write/Grep/Glob, Gemini run_shell_command/read_file/write_file/replace/grep_search/glob, Codex shell/apply_patch): Claude PreToolUse, PostToolUse, PostToolUseFailure, PermissionDenied, Stop, PreCompact, SessionStart, SessionEnd, UserPromptSubmit; Gemini BeforeTool, AfterTool, AfterAgent, PreCompress, SessionStart, SessionEnd; Codex PreToolUse, PostToolUse, Stop, PreCompact, SessionStart, UserPromptSubmit. Also one negative payload per harness, `tests/fixtures/payloads/<harness>/leaky-PostToolUse.json` (Gemini: leaky-AfterTool.json), whose tool_input carries prompt-like text, whose tool_response carries a long output, and whose paths are absolute under /Users/someone; later slices prove nothing from it leaks.

Tests you write first (unit beside the code, integration under tests/):
- harness: FromStr accepts exactly the three names; events() for each harness equals the §5 list (assert the exact members and count); has_event; session_end_event and before_tool_event; parse_payload succeeds on every fixture payload and fills session_id, event, cwd, tool_name and tool_input where the fixture has them; parse_payload on a non-object and on an object without hook_event_name is Error::Harness; render_answer produces byte-exact JSON for: Claude PreToolUse Deny, Claude Stop Block, Gemini BeforeTool Deny, Gemini AfterAgent Block, Codex PreToolUse Deny, Codex Stop Block, and Allow renders `{}`; exit_code is 0/2/2/3.
- manifest: every positive fixture in contracts/fixtures/manifest/ parses (iterate the directory, skip invalid-*); every invalid-* fixture is Error::Manifest naming the bed; the weeder fixture becomes a Bed with binary Some("weeder"), skill Some("SKILL.md"), hooks claude [PreToolUse:Bash, Stop], gemini [BeforeTool:run_shell_command, AfterAgent], codex [PreToolUse, Stop], git [pre-commit, pre-push, pre-rebase], check Some("weeder check --format sarif"); the pollen fixture becomes a Bed with binary None, no hooks, and mcp Some (command "node", args ["pollen.mjs"], env ["POLLEN_ID", "POLLEN_ALLOW"]); a manifest whose faces.mcp carries only tools becomes a Bed with mcp None; a manifest with install.binary_name "weeder-bin" and faces.cli "weeder" has binary "weeder-bin"; the invalid-git-no-rev fixture is refused by the schema; a manifest declaring hooks for harness "cursor" is Error::Manifest; a manifest declaring Claude event "BeforeTool" is Error::Manifest; a manifest declaring git hook reference-transaction is Error::Manifest; a manifest missing coverage is refused by the schema. load_beds on a temp root with two cached manifests returns them sorted by name and Error::Io-free when the beds directory is absent (empty Vec).
- lock: contracts/fixtures/garden.lock parses to season 2026.09 with judges weeder (two platforms, the sha256 values from the file), tilth, tend2 (npm Some("@plotplot/tend2")); a lock whose sha256 is not 64 lowercase hex is refused; a lock without judges is refused; read_lock on a root without garden.lock is Ok(None); platform() is a non-empty target triple that matches env!("TARGET")-style build information (use a build.rs or the `std::env::consts` pair mapped to the Rust triple; assert it equals the triple cargo built for).
- layout: every path in brief §3 is a function of the root and the documented relative path.
- version (assert_cmd): `plotplot version` in a temp dir prints exactly `plotplot 0.1.0`; with contracts/fixtures/garden.lock copied as garden.lock it prints the season and three judge lines; `plotplot --version` prints the first line; a malformed garden.lock yields exit 1 and the error on stderr with the path first.

CI: .github/workflows/stem.yml runs on push and pull_request for paths Cargo.toml, Cargo.lock, src/**, tests/**, contracts/** on ubuntu-latest and macos-latest: checkout, dtolnay/rust-toolchain@stable with components rustfmt and clippy, then the three gate commands. Reference no secrets.

Do NOT create: src/bundle/, src/deny.rs, src/friction.rs, src/receipt.rs, src/hook.rs, src/doctor.rs, src/plant/, scripts/fit/stem.sh, or any CLI face other than version. Do NOT add a SEASON constant: the season is the lock's.
"""
...
SMOKE_INTEGRATE = GATES + " && bash scripts/fit/stem.sh bundles && bash scripts/fit/stem.sh hook-faces"

AUDIT_INTEGRATE = (
    "cargo test 2>&1 | tail -20 && bash scripts/fit/stem.sh hook-faces && bash scripts/fit/stem.sh bundles "
...
plan = {
    ...
    "maxConcurrency": 2,
    "nodes": [
        node("stem.crate", CRATE),
        node("stem.bundle", BUNDLE, needs=["stem.crate"]),
        node("stem.faces", FACES, needs=["stem.crate"]),
        node("stem.plant", PLANT, needs=["stem.crate"]),
