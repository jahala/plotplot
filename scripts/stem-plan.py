...
- manifest: every positive fixture in contracts/fixtures/manifest/ parses (iterate the directory, skip invalid-*); every invalid-* fixture is Error::Manifest naming the bed; the weeder fixture becomes a Bed with binary "weeder", skill Some("SKILL.md"), hooks claude [PreToolUse:Bash, Stop], gemini [BeforeTool:run_shell_command, AfterAgent], codex [PreToolUse, Stop], git [pre-commit, pre-push, pre-rebase], check Some("weeder check --format sarif"); the pollen fixture becomes a Bed with mcp None and no hooks; a manifest declaring hooks for harness "cursor" is Error::Manifest; a manifest declaring Claude event "BeforeTool" is Error::Manifest; a manifest declaring git hook reference-transaction is Error::Manifest; a manifest missing coverage is refused by the schema. load_beds on a temp root with two cached manifests returns them sorted by name and Error::Io-free when the beds directory is absent (empty Vec).
