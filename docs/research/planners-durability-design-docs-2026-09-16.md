# Four readings: [vendor], [vendor], [vendor], and how to write a design doc

Read on 2026-09-16 at the owner's request. Sources: [vendor] (README, the ADR list,
docs/why-a-separate-planner.md), [vendor] (README), [vendor] (the product page),
...
## [vendor]: a planner that only reads, a plan that is a file, a verdict that is a marker

[vendor] turns one goal into an ordered plan of coding-agent tasks, each with its own runner,
...
## [vendor]: durable execution as a queue plus a state store, on one database

[vendor] is durable execution on Postgres alone: a task is decomposed into steps, each step's result
...
- **Code outside a step may run more than once.** [vendor] says so on the first page. pleach's
...
- **First emit wins.** umbel 86 (read at Stop races the transcript) is an event race of exactly
  the kind [vendor] removes by caching the first emit. The runner contract now says stop means
...
## [vendor]: a document that runs

[vendor] packs an application, its data and its media into one portable file that opens offline
