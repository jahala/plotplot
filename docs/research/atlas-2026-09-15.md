# What atlas has that the garden should take, and what it must not

Read on 2026-09-15 at the owner's request: pacifio/atlas, "source control for coding agents"
(Rust and Tauri, MIT, 4.5k stars, pushed the same day). Sources: its README, ARCHITECTURE.md,
ADR-0001 to ADR-0008, and the `atlas-checkpoint` and `atlas-redact` crates. Compared against
...
atlas is a desktop app that runs Claude Code, Codex and any ACP-registry agent side by side,
...
atlas scrubs secrets on the way into its store, never on the way out. `atlas-redact` is a
...
atlas reports capture health as one of three states per workspace, OK, Degraded or Stopped,
...
atlas observes commits rather than intercepting them (no hooks, so a commit made before the
...
- **ACP as the drive path.** atlas runs Claude Code and Codex over the Agent Client Protocol
...
- **Session handoff fact pack.** atlas assembles a curated fact pack plus the tail of the last
