...
Brief for the agent building weed. Self-contained: you have not seen any prior conversation.
Read in this order before writing code: `~/conductor/workspaces/plotplot-ai/cape-town-v1/docs/building-the-garden.md`
(the law every bed agent follows), `~/conductor/workspaces/plotplot-ai/cape-town-v1/docs/tend2/weed.tend2.html`
...
weed is the judge of the diff. A static binary that reads what an agent produced and refuses
...
with zero tokens. Agents produce; weed decides.

Three faces on one core:

- `weed check` judges a **diff** and may block. Pre-commit, Stop hook, pleach's smoke gate, CI.
- `weed scan` judges the **tree** and never blocks. Repo-state hygiene: a cited path or symbol
  that no longer exists, dead exports, TODO age, dependency lag. tend2's hygiene loop is its
  first consumer.
- `weed guard` is the **law** in git: hooks installed through `core.hooksPath` that run
  `check` at commit and push and refuse history rewrites and non-fast-forward pushes to
  protected branches.

Plus `weed bite`, the dynamic rule, once pleach commits the test phase separately: apply the
...
- No TODOs, stubs, or fallbacks in committed code; weed would flag them. No time estimates.
...
**Its own repository**, the `weed` repo (builder workspace
`~/conductor/workspaces/weed/bandung`), as a Rust crate with its own binary `weed`, its own
...
a library dependency is not shared state, so weed still passes the standalone rule; and
separate repositories keep that rule honest. (Decided 2026-09-05: the earlier draft placed
weed inside the tilth Cargo workspace; the owner created a separate repository, which the
...
tilth's suite still green. Report over pollen before building weed on top; the tilth agent
owns that landing.

## Shape

**Core** (`weed/src/core/`): pure. `parse_diff → Hunks`, `classify_file(path, content) →
...
**Seams** (`weed/src/seams/`): git (diff, file at HEAD and in tree, refs, hooks path), exec
(run a test command with a timeout), fs. Injected; never imported by core.

**Faces** (`weed/src/faces/`):

```
weed check [--base <ref>] [--staged] [--scope <glob>...] [--strict] [--format sarif|table]
weed scan  [--rules <ids>] [--format sarif|table]
weed guard install | status | uninstall          git hooks via core.hooksPath
weed bite  --test "<command>" [--base <ref>]      Part 5
weed hook  <claude|gemini|codex>                  harness hook JSON on stdin
weed rules                                        the catalogue with severities and faces
```
...
**Exit codes:** 0 clean or warnings only; 2 at least one block-level result; 3 weed could not
...
**Config:** `weed.toml` at the repo root, optional. Per rule: `off | warn | block` for check
...
binary; `weed rules` prints them.

**Suppression:** commit trailer `Weed-allow: T2 <reason>` or inline `weed-allow T2: <reason>`.
...
**pleach.** `examples/pleach/plan.json` with `"accept": { "smoke": "weed check --strict" }`;
validate it with `pleach validate`. Document in `docs/pleach.md` that weed sees the staged
files plus the tree against HEAD, so no base argument is needed.

**guard.** `weed guard install` writes `.githooks/pre-commit` (`weed check --staged --strict`),
`.githooks/pre-push` (`weed check --strict --base <upstream>` plus refusal of non-fast-forward
pushes to protected branches from `weed.toml`), and `.githooks/pre-rebase` (refuse rewriting
...
**hooks.** `weed hook claude` handles `PreToolUse` for `Bash` when the command contains
`git commit` (check `--staged`, deny with the SARIF results rendered as the reason) and
`Stop` (check the working tree; block the stop with the findings). `weed hook gemini` handles
`BeforeTool` the same way. `weed hook codex`: verify what Codex's hook events can deny now,
...
**CI.** `examples/ci/github.yml`: `weed check --base origin/${{ github.base_ref }} --strict
--format sarif > weed.sarif` uploaded with `github/codeql-action/upload-sarif`, so results are
code-scanning alerts. This Action is weed's first distribution: maintainers drowning in
agent-authored pull requests.

**tend2 seam.** `docs/tend2-seam.md`: how tend2's hygiene loop cites `weed scan --format
sarif` output as evidence, and how tend2's verifier could call `weed check` for its mock-only
refusal instead of carrying `mock.ts`. Proposal only; do not edit tend2.

## Calibration and the kill criterion

1. `scripts/calibrate.rs` (or a `cargo xtask`) runs `weed check --base <parent> --strict`
...
weed is deterministic by design, and "good" for a deterministic gate has six measurable
parts. Each is a check on weed's umbrella loop with an evidence script.
...
2. **Recall, not only precision.** Calibration measures false blocks on real merged commits.
   ...
   body, swallow an error, add a secret, edit a workflow file), and weed must fire on exactly
...
3. **Never panic, fail closed.** Malformed diffs, binaries, non-UTF-8, CRLF, renames,
   ...
   is a hole.
4. **Latency budget.** `weed check --staged` under 200 ms on a typical diff in the fixture
...
5. **The builder does not grade its own calibration.** The classification of blocked commits
   is the number weed ships on. An independent party, a pleach audit node on a different
   ...
   and redone.
6. **Watch for routing around it.** Suppression rate, `Weed-allow` trailers per hundred
...
The umbrella verifies weed against the fit contract from `docs/building-the-garden.md` §3
(F1 to F6) with `scripts/fit/weed.sh`. To pass it, ship: `garden.json` (kind: gate and hook;
zero declared context cost; metric command: calibration precision; check command: `weed
check --format sarif`); `SKILL.md` with a one-sentence description; `docs/tend2/` inside the
crate with your module loops; `.brand/products/weed/` in the pleach shape with the name and
...
- `weed check` on the tilth workspace tree at the end reports nothing at block level.
...
Most of weed's individual rules exist somewhere as a single-language or single-concern tool.
weed's unclaimed ground is the honesty of the **diff against tests and guardrails**, the cheap
effectiveness proof (`bite`), the placement at the agent's Stop and at pleach's node, and the
calibration discipline. Where a rule below has a mature home, weed either reuses it or keeps
its own only because the cross-language, diff-time framing needs it.

| weed rule or face | Existing open-source tools | Consequence for weed |
|---|---|---|
...
| X2 blast radius | OpenLore `change_impact_certificate` and `certify_public_surface` | weed's X2 is scope enforcement from a plan, which OpenLore does not do; the blast-radius text can come from tilth-core |
...
| T3 skips and focus | ESLint `jest/no-disabled-tests`, `no-focused-tests`; ruff PT rules | those flag any skip in a file; weed flags an **added** skip in the diff, a different question; keep |
| T1, T2, T4, T5, T6, T7 test tampering | none found as a deterministic gate; GitHub's own review guidance says to watch for it by eye; PyTorch's playbook checks it manually; pyor.review sells a model-based reviewer | weed's core; this is the unclaimed ground |
| C1 guardrail edits, X2 scope from a plan | none found | weed's core |
| `bite` | mutation testing (Stryker, mutmut, cargo-mutants) as the heavyweight cousin | weed's cheap revert-and-run is distinct; keep |
| `guard`, hooks, SARIF, Action | husky, lefthook, pre-commit framework; aislop and OpenLore both ship hooks, SARIF and Actions | plumbing is commodity; weed's version exists so the honesty rules run at commit and at Stop, not for its own sake |
| PR-level slop triage | anti-slop (813 stars, AGPL): 34 checks on branch, size, title, description, commits, contributor history; language-agnostic; auto-closes | a different layer, metadata not diff content; complementary; recommend it to maintainers alongside weed |
